...
/**
 * Idioma de PARTIDA do checkout a partir do código de país ISO-3166 do IP
 * (header `x-vercel-ip-country`, fornecido de graça pela Vercel). SÓ o Brasil
 * entra no fluxo pt/BR; qualquer outro país — inclusive ausente, desconhecido
 * ou Portugal — cai no fluxo padrão es/AR, preservando o comportamento legado
 * argentino. É apenas o *default*: o `?lang=` escolhido pelo visitante sempre
 * vence (ver a decisão de precedência em middleware.ts).
 */
export function idiomaPorPaisIp(countryCode) {
  return String(countryCode ?? "").trim().toUpperCase() === "BR" ? "pt" : "es";
}
...
/**
 * Configuração de Intl.NumberFormat a partir da MOEDA já gravada no registro de
 * pagamento (coluna assinaturas.moeda), usada pelo painel admin para exibir cada
 * linha na moeda correta:
 *   - BRL → locale pt-BR, com centavos (ex.: R$ 19,89)
 *   - ARS → locale es-AR, sem centavos (valores "cheios", ex.: $ 11.980)
 * Qualquer valor fora de {BRL, ARS} cai em ARS (fluxo legado seguro), coerente
 * com o default 'ARS' da coluna.
 */
export function configFormatoMoeda(moeda) {
  return String(moeda ?? "").trim().toUpperCase() === "BRL"
    ? { locale: "pt-BR", currency: "BRL", maximumFractionDigits: 2 }
...
export const PRECOS_ASSINATURA = {
  basico: {
    ...
    BR: { mensal: 19.89, anual: 198.9 },
  },
  pro: {
    AR: { mensal: 19850, anual: 198500 },
    BR: { mensal: 27.49, anual: 274.9 },
  },
};
