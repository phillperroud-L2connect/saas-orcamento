...
export const BLOCOS_TEMPLATE = {
  ...
  // Swiss Studio (3c): orçamento real completo, mesma sequência do Atelier/
  // Blueprint. banco_horas/projetos/condicoes seguem no catálogo mas nascem
  // ocultos (ver BLOCOS_PADRAO_OCULTOS) por não terem fonte de dado no formulário.
  swiss_studio: [
    "cabecalho",
    "cliente",
    "servicos",
 ...
 * básicos). Os 3 premium já foram convertidos (3a/3b/3c) e escondem os mesmos
 * três blocos sem fonte de dado.
 */
export const BLOCOS_PADRAO_OCULTOS = {
  ...
  swiss_studio: ["banco_horas", "projetos", "condicoes"],
};
...
/**
 * O tenant com este plano pode esconder/mostrar blocos (feature exclusiva do
 * Plano Pro)?
 *
 * Mesma regra estrita do gating de template premium (podeUsarTemplate em
 * lib/templates-core.js): SÓ `plano === "pro"`. Qualquer outro valor (`basico`,
 * `manual`, nulo/desconhecido) é negado — negar é o lado seguro e nunca herda.
 * Espelhar a regra é proposital: a UI de toggles e os templates premium são o
 * mesmo pacote do Pro, então os dois gates precisam decidir igual.
 * @param {unknown} plano
 * @returns {boolean}
 */
export function podeEsconderBlocos(plano) {
  return String(plano ?? "") === "pro";
}
